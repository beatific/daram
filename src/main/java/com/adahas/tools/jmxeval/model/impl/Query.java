  ...
  public void process(final Context context) throws EvalException {
    try {
      ...
      if (compositeAttribute == null) {
    	if(RegularExpressionUtil.commaMatcher(attribute).find()) {
    		
    		String[] attributes = attribute.split(",");
    		attributeValues = new Object[attributes.length];
    		int index = 0;
    		for(String attr : attributes) {
    			attributeValues[index++] = context.getConnection().getAttribute(mbeanName, attr);
    		}
    	} else {
    		Object attributeVal = context.getConnection().getAttribute(mbeanName, attribute);
      ...
      if(RegularExpressionUtil.commaMatcher(var).find()) {
    	  int i = 0;
    	  for(String v : var.split(",")) {
    	      context.setVar(v, attributeValues[i++]);
    	  }
      } else {
