...
define(['library/websocket/socketjs-0.3.4.min', 'library/websocket/stomp'], function () {

	function _controller($scope, $http, websocket) {

		$scope.$emit('updateCSS', ['stylesheets/daram.css', 'stylesheets/hangul.css', '//code.jquery.com/ui/1.11.2/themes/smoothness/jquery-ui.css']);
		...
		websocket.receive().then(null, null, function(list) {
		...
		$scope.select = function(design, isRegister) {
		    ...
		    if(isRegister)websocket.send($scope.selected);
